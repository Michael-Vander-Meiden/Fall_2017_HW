...
class FCN(object):
    def __init__(self, architecture=[784,100,784], eta = 0.1, batch_size = 1, n_epochs=150, rand_seed = 45, momentum = .9, dropout=False):
        ...
        for epoch in range(self.n_epochs):
            ...
            for mini_batch in mini_batches:
                ...
                for x, y in mini_batch:
                    if self.dropout:
                        mask = np.squeeze(np.random.binomial([np.ones((784,1))],0.9),axis=0)
                        x = x*mask
                        
                    self._forward_prop(x)
                    delta_nabla_b, delta_nabla_w = self._back_prop(x, x)
            ...
            tcross_entropy = [self.cross_entropy(x,x) for x,y in training_data]
            vcross_entropy = [self.cross_entropy(x,x) for x,y in validation_data]
